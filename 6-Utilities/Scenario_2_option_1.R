...
# now check which low index VME index cells are unfished
  ...
  VMEdepth$VME_Class[VMEdepth$VME_Class == 0 & VMEdepth$SAR < 0.43] <- 5 # low index with low SAR
  
  VME_high <- VMEdepth[VMEdepth$VME_Class %in% c(3,2,1,5),]
...
# select all VME low index that are adjacent/joining
  ...
  VME_low$joining <- NA
  VME_low$long <- round(VME_low$long, digits = 5)
  VME_low$lat <- round(VME_low$lat, digits = 5)
  ...
  for (i in 1:nrow(VME_high)){
    ...
    sublong <- round(sublong, digits = 5)
    sublat  <- c(lat-(0.05),lat+(0.05),lat)
    sublat <- round(sublat, digits = 5)
...
## and make leaflet map
  ...
  VME_low <- cbind(VME_low, vmsall[match(VME_low@data$csquares,vmsall$c_square), c("SAR")])
  colnames(VME_low@data)[ncol(VME_low@data)] <- "SAR"
  VME_low_lowF <- subset(VME_low,VME_low@data$SAR <0.43)
  VME_low_highF <- subset(VME_low,VME_low@data$SAR >= 0.43)
  ...
  mfs <- leaflet() %>%
    ...
    addPolygons(data = clos21, group = "Closures",
    ...
    addPolygons(data = VME_low_lowF, group = "VME index - low, SAR <0.43",
                stroke = FALSE, fillOpacity = 1, smoothFactor = 0.5,fillColor =   "#df65b0") %>%
    addPolygons(data = VME_low_highF, group = "VME index - low, SAR >=0.43",
    ...
    addLayersControl(
      overlayGroups = c("Depth 400-800 m", "Closures",
                        ...
                        "VME index - medium","VME index - low, SAR <0.43","VME index - low, SAR >=0.43"),
...
# save output
  ...
  setwd(outdir)
  saveWidget(mfs, file="Scenario2_option1.html")
  
  outdir <- paste(pathdir,"2-Data processing",sep="/") 
  write_sf(reg_dropped, paste(outdir,"Scenario2_option1.shp",sep="/"))
