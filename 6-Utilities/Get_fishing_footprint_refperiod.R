...
# run to get fishing footprint
  ...
  indexcol_footprint <- which(names(vmsreg_footprint) %in% nam) 
  vmsreg_footprint$ref <- rowSums(vmsreg_footprint[indexcol])
  vmsreg_footprint$ref[vmsreg_for_footprint$ref > 0] <- 1
  ...
  rm(list=setdiff(ls(), c("pathdir","pathdir_nogit" , "EcoReg", "Footprint")))
