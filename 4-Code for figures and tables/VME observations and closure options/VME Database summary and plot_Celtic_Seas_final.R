...
#-------------------------------------------------------------------------------------
# closure scenarios directory (NB all but 2.2 invalid geoms)
...
foi.sp$`VME indicator type` = foi.sp$`VME indicator type`[sort(names(foi$`VME indicator type`))][1:10]
foi.close.sc1$`VME indicator type` = foi.close.sc1$`VME indicator type`[sort(names(foi$`VME indicator type`))][1:10]
foi.close.sc1.opt2$`VME indicator type` = foi.close.sc1.opt2$`VME indicator type`[sort(names(foi$`VME indicator type`))][1:10]
foi.close.sc2.opt1$`VME indicator type` = foi.close.sc2.opt1$`VME indicator type`[sort(names(foi$`VME indicator type`))][1:10]
foi.close.sc2.opt2$`VME indicator type` = foi.close.sc2.opt2$`VME indicator type`[sort(names(foi$`VME indicator type`))][1:10]
# check
lapply(list(foi, foi.sp, foi.close.sc1, foi.close.sc1.opt2, foi.close.sc2.opt1, foi.close.sc2.opt2), function(i) names(i$`VME indicator type`))
...
# there is probably a better way of doing this, but short on time so put in a quick fix here - no anemone or stalked crinoids in sc2.2
foi$`VME habitat type` = foi$`VME habitat type`[c(sort(names(foi$`VME habitat type`))[2:6], sort(names(foi$`VME habitat type`))[c(8:9,1)], "Stalked crinoid aggregations")]
foi.sp$`VME habitat type` = foi.sp$`VME habitat type`[c(sort(names(foi$`VME habitat type`))[2:6], sort(names(foi$`VME habitat type`))[c(8:9,1)])]
foi.close.sc1$`VME habitat type` = foi.close.sc1$`VME habitat type`[c(sort(names(foi$`VME habitat type`))[2:6], sort(names(foi$`VME habitat type`))[c(8:9,1)])]
foi.close.sc1.opt2$`VME habitat type` = foi.close.sc1.opt2$`VME habitat type`[c(sort(names(foi$`VME habitat type`))[2:6], sort(names(foi$`VME habitat type`))[c(8:9,1)])]
foi.close.sc2.opt1$`VME habitat type` = foi.close.sc2.opt1$`VME habitat type`[c(sort(names(foi$`VME habitat type`))[2:6], sort(names(foi$`VME habitat type`))[c(8:9,1)])]
foi.close.sc2.opt2$`VME habitat type` = foi.close.sc2.opt2$`VME habitat type`[c(sort(names(foi$`VME habitat type`))[2:6], sort(names(foi$`VME habitat type`))[8:9])]
# check
lapply(list(foi, foi.sp, foi.close.sc1, foi.close.sc1.opt2, foi.close.sc2.opt1, foi.close.sc2.opt2), function(i) names(i$`VME habitat type`))
...
#-------------------------------------------------------------------------------------
# write directory
SumStatsdir = paste0(wd, "/Scenarios and VMEs/FINAL/Celtic Sea Ecoregion/")
...
if(!file.exists(paste0(SumStatsdir, "VME_Indicator_summary_NoR_all_areas_closures_Celtic_Sea.csv")) || recompute) {
  write.csv(Ind.hist.df, file = paste0(SumStatsdir, "VME_Indicator_summary_NoR_all_areas_closures_Celtic_Sea.csv"))
}
Ind.hist.df 
Ind.hist.df = Ind.hist.df[c(1:4,6:10),]
Ind.hist.df
Ind.hist.df_long = melt(Ind.hist.df, id.vars = "VME_Indicator")
names(Ind.hist.df_long) = c("VME_Indicator", "Area", "value")

# Plot object
vme.ind.hist = lapply(unique(Ind.hist.df_long$VME_Indicator), function(i){
  ggplot(data=Ind.hist.df_long[Ind.hist.df_long$VME_Indicator == i,] , aes(x=VME_Indicator, y=value, fill = Area)) +
...
# plot
png(filename = paste0(SumStatsdir, "Celtic_Seas_VME_Indicator_bar_plots.png"),
    width = 3600, height = 3600, units = "px", pointsize = 12,
    bg = "white", res = 300, family = "", restoreConsole = TRUE,
    type = c("cairo-png"),
    symbolfamily="default")
plot_grid(vme.ind.hist[[1]], vme.ind.hist[[2]], vme.ind.hist[[3]], vme.ind.hist[[4]], vme.ind.hist[[5]], vme.ind.hist[[6]], vme.ind.hist[[7]], vme.ind.hist[[8]], vme.ind.hist[[9]], ncol = 3, nrow = 3)
dev.off()

# Habitats
Hab.hist.df = data.frame(
  HabitatType = c(plyr::ddply(vme.cs@data, .(HabitatType), summarise, NoR = length(HabitatType))[2:6,1], plyr::ddply(vme.cs@data, .(HabitatType), summarise, NoR = length(HabitatType))[c(8:9,1,7),1]),
  ER = c(plyr::ddply(vme.cs@data, .(HabitatType), summarise, NoR = length(HabitatType))[2:6,2], plyr::ddply(vme.cs@data, .(HabitatType), summarise, NoR = length(HabitatType))[c(8:9,1,7),2]),
  DB = c(plyr::ddply(vme.dbnd2@data, .(HabitatType), summarise, NoR = length(HabitatType))[2:6,2], plyr::ddply(vme.cs@data, .(HabitatType), summarise, NoR = length(HabitatType))[c(8:9,1),2], NA),
  Sc1.1 = c(plyr::ddply(vme.close.sc1@data, .(HabitatType), summarise, NoR = length(HabitatType))[2:6,2], plyr::ddply(vme.cs@data, .(HabitatType), summarise, NoR = length(HabitatType))[c(8:9,1),2], NA),
  Sc1.2 = c(plyr::ddply(vme.close.sc1.opt2@data, .(HabitatType), summarise, NoR = length(HabitatType))[2:6,2], plyr::ddply(vme.cs@data, .(HabitatType), summarise, NoR = length(HabitatType))[c(8:9,1),2], NA),
  Sc2.1 = c(plyr::ddply(vme.close.sc2.opt1@data, .(HabitatType), summarise, NoR = length(HabitatType))[2:6,2], plyr::ddply(vme.cs@data, .(HabitatType), summarise, NoR = length(HabitatType))[c(8:9,1),2], NA),
  Sc2.2 = c(plyr::ddply(vme.close.sc2.opt2@data, .(HabitatType), summarise, NoR = length(HabitatType))[,2], NA, NA)
)
if(!file.exists(paste0(SumStatsdir, "VME_Habitat_summary_NoR_all_areas_closures_Celtic_Sea.csv")) || recompute) {
  write.csv(Hab.hist.df, file = paste0(SumStatsdir, "VME_Habitat_summary_NoR_all_areas_closures_Celtic_Sea.csv"))
}
Hab.hist.df 
Hab.hist.df = Hab.hist.df[c(1:3,5:9),] # REMOVE NULLS
Hab.hist.df 
...
# plot
png(filename = paste0(SumStatsdir, "Celtic_Seas_VME_Habitat_bar_plots.png"),
    width = 3600, height = 3600, units = "px", pointsize = 12,
    bg = "white", res = 300, family = "", restoreConsole = TRUE,
    type = c("cairo-png"),
    symbolfamily="default")
plot_grid(vme.Hab.hist[[1]], vme.Hab.hist[[2]], vme.Hab.hist[[3]], vme.Hab.hist[[4]], vme.Hab.hist[[5]], vme.Hab.hist[[6]], vme.Hab.hist[[7]], vme.Hab.hist[[8]], ncol = 3, nrow = 3)
dev.off()
...
CS.bm = raster("E:/ICES - WKEUVME/R work/Celtic_Sea.GEBCO.base.map_utm29n.tif")
# hillshade
...
png(filename = paste0(SumStatsdir, "Celtic Seas_scenarios_eg_sponge_indicator_6_panel.png"),
